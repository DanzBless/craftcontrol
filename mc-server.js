...
class MinecraftServer {
  ...
  generateJvmArgs() {
    ...
    const content = `# Generated by CraftControl Universal\n-Xms${this.config.minRam} -Xmx${this.config.maxRam}\n`;
    try {
  ...
  parseLogLine(line) {
    ...
    if (javaReqMatch || classVerMatch) {
      ...
      this.appendLog(`[CraftControl Auto-Fix] Detected Java version mismatch (requires Java ${requiredMajor}+). Resolving matching Java runtime...`);
      const optimalJava = JavaResolver.resolveJavaByMajor(requiredMajor);
      if (optimalJava && optimalJava !== this.config.javaPath) {
        this.appendLog(`[CraftControl Auto-Fix] Auto-switching Java path from "${this.config.javaPath}" to "${optimalJava}".`);
        this.saveConfig({ javaPath: optimalJava });
        setTimeout(() => {
          if (this.status === 'offline') {
            this.appendLog('[CraftControl Auto-Fix] Relaunching server with correct Java runtime now...');
            this.start();
          }
        }, 1500);
      }
  ...
  start() {
    ...
    if (serverType === 'bedrock') {
      ...
      spawnCmd = exePath;
      spawnArgs = [];
      this.appendLog(`[CraftControl] Starting Bedrock Dedicated Server: ${exePath}`);
    } else {
      ...
      if (!javaCmd || !fs.existsSync(javaCmd) || (currentMajor && currentMajor < requiredMajor)) {
        ...
        this.appendLog(`[CraftControl Auto-Fix] Selected Java (${currentMajor || 'unknown'}) is incompatible with Minecraft ${detectedMcVersion} (needs Java ${requiredMajor}+). Auto-switching to: ${optimalJava}`);
        javaCmd = optimalJava;
      ...
      this.appendLog(`[CraftControl] Starting Java Server (${serverType}): ${javaCmd} ${spawnArgs.join(' ')}`);
    }
    ...
    try {
      ...
      this.process.on('close', (code) => {
        this.appendLog(`[CraftControl] Server process exited with code ${code}`);
        const wasOnline = this.status === 'online';
        this.setStatus('offline');
        this.process = null;

        if (wasOnline && this.config.autoRestart && code !== 0) {
          this.appendLog('[CraftControl] Auto-restart enabled. Restarting in 5 seconds...');
          setTimeout(() => {
      ...
      this.process.on('error', (err) => {
        this.appendLog(`[CraftControl ERROR] Process failed: ${err.message}`);
        this.setStatus('offline');
        this.process = null;
      });
  ...
  stop() {
    ...
    this.setStatus('stopping');
    this.appendLog('[CraftControl] Sending stop command to server...');
    ...
    setTimeout(() => {
      if (this.status === 'stopping' && this.pid === targetPid) {
        this.appendLog('[CraftControl] Server graceful shutdown timed out. Terminating...');
        this.kill();
      }
    }, 25000);
  ...
  restart() {
    ...
    this.appendLog('[CraftControl] Server restart requested...');
  ...
  saveServerProperties(newProps) {
    ...
    content += `#Updated by CraftControl Universal on ${new Date().toISOString()}\n`;
